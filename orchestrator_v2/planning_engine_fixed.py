...
class PlanningEngineFixed:
    ...
    async def _parse_llm_response(self, response_text: str, context: PlanningContext) -> Optional[ExecutionPlan]:
        ...
        try:
            ...
            return ExecutionPlan(
                ...
                steps=steps,
                strategy=plan_data.get('strategy', 'adaptive'),
    ...
    async def _fix_common_parameter_issues(self, tool_name: str, parameters: Dict, context: PlanningContext) -> Dict:
        ...
        # Fix missing doc_name parameter
        if tool_name == 'search_uploaded_docs' and 'doc_name' not in fixed_params:
            if context.active_documents:
                ...
                fixed_params['doc_name'] = "any_document"
                logger.debug("Added placeholder doc_name parameter")
        
        # Fix search tools parameters
        if tool_name in ['search_conversation_history', 'search_knowledge_base']:
    ...
    async def _create_template_plan(self, context: PlanningContext, strategy: PlanningStrategy) -> ExecutionPlan:
        ...
        if query_type == QueryType.SEARCH:
            steps = await self._create_search_template_steps(context)
        elif query_type == QueryType.ANALYSIS:
            steps = await self._create_analysis_template_steps(context)
        elif query_type == QueryType.SUMMARY:
            steps = await self._create_summary_template_steps(context)
        else:
            steps = await self._create_general_template_steps(context)
    ...
    async def _create_search_template_steps(self, context: PlanningContext) -> List[ExecutionStep]:
        ...
        # Step 1: Search uploaded documents if available
        if context.active_documents:
    ...
    async def _create_simple_fallback_plan(self, context: PlanningContext) -> ExecutionPlan:
        ...
        return ExecutionPlan(
            ...
            description="Simple fallback plan",
    ...
    async def _validate_plan_with_fixes(self, plan: ExecutionPlan, context: PlanningContext) -> PlanValidationResult:
        ...
        try:
            ...
            for step in plan.steps.values():
                ...
                if step.tool_name not in self.tool_registry.get_available_tools():
    ...
    def _create_planning_prompt(self, context: PlanningContext, strategy: PlanningStrategy) -> str:
        """Create planning prompt for LLM with clear condition guidelines."""
...
IMPORTANT CONDITION GUIDELINES:
...
- For custom conditions, keep them simple and use CUSTOM type
...
Create a plan that addresses the user query effectively using the available tools."""
